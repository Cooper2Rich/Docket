...
- [Activated issue setup](../docs/implementation/setup-report.md) - Verified GitHub queue, 42 external-controller tests, protected bootstrap/main policy and exact launch guide.
...
- [[build-readiness|Build readiness]] - Build and Sol/high authorized; all ten foundation leaves are integrated, while the first identity-access leaf is a controller-prepared candidate pending its final-head evidence, PR, required checks, and protected integration.
- [[access-model|Access model]] - Clerk-managed authentication and Sessions, the current first-slice identity implementation candidate, Clerk-recognized verified-email invitation matching, PostgreSQL-backed scoped authorization, School authority and affiliation, and Tournament staff authority.
