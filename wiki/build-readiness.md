...
On September 9, 2026, the owner requested deletion of the previous implementation and organization of the requirements and build instructions. The owner later authorized the Release 1 build on September 23. `R1-FND-001-A` through `R1-FND-004-A` are integrated into the protected bootstrap branch. The eighth bounded leaf now adds a canonical registry for all 460 active acceptance criteria, generated machine and readable traceability indexes, explicit accounting for 98 deferred or superseded source rows, and fail-closed coverage, artifact ownership, stale-hash and golden-corpus validation.

The [work graph](../docs/implementation/work-graph.yaml) preserves 40 objectives as tracking groups and defines 115 implementation leaves, 460 acceptance IDs and eight decision/external gates. The controller-prepared candidate marks `R1-FND-004-B` done and `R1-FND-005-A` ready only if the current leaf receives a reviewable PR, exact-head evidence, protected `Bootstrap / Verify`, and controller-verified merge into `codex/release-1-bootstrap`; no dependent work may start yet.
...
The external launcher accepts explicit reasoning, subagent-disable and quiet-output parameters. The Docket controller enforces Sol/high, one leaf, complete evidence, exact-head CI, controller-evidence review, protected integration and restart-safe issue reconciliation. Forty-two controller/fake-worker tests cover bootstrap/promotion routing and zero-account-review fixtures. The first seven bounded leaves are integrated through PRs #168–#174; the eighth has prepared its candidate transition but has not yet established protected integration. All 163 issue bodies, 115 parent links and 122 blocking relationships were synchronized against the initial contracts. The [setup report](../docs/implementation/setup-report.md) and [evidence record](../docs/implementation/setup-evidence.json) record the setup and activation state.
