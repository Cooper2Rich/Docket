...
On September 9, 2026, the owner requested deletion of the previous implementation and organization of the requirements and build instructions. The owner later authorized the Release 1 build on September 23. `R1-FND-001-A`, `R1-FND-001-B`, `R1-FND-002-A`, and `R1-FND-002-B` are integrated into the protected bootstrap branch. The fifth bounded leaf now adds an ordered module-owned migration plan, immutable PostgreSQL journal, pre-startup ownership and compatibility validation, explicit migration process, isolated database harness, and retained rollback-failure evidence in [PR #172](https://github.com/Cooper2Rich/Docket/pull/172).

The [work graph](../docs/implementation/work-graph.yaml) preserves 40 objectives as tracking groups and defines 115 implementation leaves, 460 acceptance IDs and eight decision/external gates. The controller-prepared candidate marks `R1-FND-003-A` done and `R1-FND-003-B` ready only if PR #172 passes exact-head evidence, protected `Bootstrap / Verify`, and controller-verified merge into `codex/release-1-bootstrap`; no dependent work may start yet.
...
The external launcher accepts explicit reasoning, subagent-disable and quiet-output parameters. The Docket controller enforces Sol/high, one leaf, complete evidence, exact-head CI, controller-evidence review, protected integration and restart-safe issue reconciliation. Forty-two controller/fake-worker tests cover bootstrap/promotion routing and zero-account-review fixtures. The first four bounded leaves are integrated through PRs #168–#171; the fifth created PR #172 and prepared its candidate transition. All 163 issue bodies, 115 parent links and 122 blocking relationships were synchronized against the initial contracts. The [setup report](../docs/implementation/setup-report.md) and [evidence record](../docs/implementation/setup-evidence.json) record the setup and activation state.
