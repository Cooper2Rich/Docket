...
On September 9, 2026, the owner requested deletion of the previous implementation and organization of the requirements and build instructions. The owner later authorized the Release 1 build on September 23. The first bounded leaf, `R1-FND-001-A`, is integrated into the protected bootstrap branch. The second leaf now adds a React Router SSR route, the pinned shadcn Base UI/base-nova foundation, deterministic Playwright accessibility coverage, and leaf acceptance verification in [PR #169](https://github.com/Cooper2Rich/Docket/pull/169).

The [work graph](../docs/implementation/work-graph.yaml) preserves 40 objectives as tracking groups and defines 115 implementation leaves, 460 acceptance IDs and eight decision/external gates. The controller-prepared candidate marks `R1-FND-001-B` done and `R1-FND-002-A` ready only if PR #169 passes exact-head evidence, protected `Bootstrap / Verify`, and controller-verified merge into `codex/release-1-bootstrap`; no dependent work may start yet.
...
The external launcher accepts explicit reasoning, subagent-disable and quiet-output parameters. The Docket controller enforces Sol/high, one leaf, complete evidence, exact-head CI, controller-evidence review, protected integration and restart-safe issue reconciliation. Forty-two controller/fake-worker tests cover bootstrap/promotion routing and zero-account-review fixtures. The first real bounded worker run integrated PR #168; the second created PR #169 and prepared its candidate transition. All 163 issue bodies, 115 parent links and 122 blocking relationships were synchronized against the initial contracts. The [setup report](../docs/implementation/setup-report.md) and [evidence record](../docs/implementation/setup-evidence.json) record the setup and activation state.
