...
On September 9, 2026, the owner requested deletion of the previous implementation and organization of the requirements and build instructions. The owner later authorized the Release 1 build on September 23. The first bounded leaf, `R1-FND-001-A`, now has a pinned pnpm/Nx workspace, accepted app and package shells, executable build/check/unit commands, boundary enforcement, a fail-closed item verifier, and rendered foundation-state coverage in [PR #168](https://github.com/Cooper2Rich/Docket/pull/168).

The [work graph](../docs/implementation/work-graph.yaml) preserves 40 objectives as tracking groups and defines 115 implementation leaves, 460 acceptance IDs and eight decision/external gates. The controller-prepared candidate marks `R1-FND-001-A` done and `R1-FND-001-B` ready only for PR validation. Exact-head acceptance evidence, protected `Bootstrap / Verify`, and controller-verified merge into `codex/release-1-bootstrap` remain the completion boundary; no dependent work may start yet.
...
The external launcher accepts explicit reasoning, subagent-disable and quiet-output parameters. The Docket controller enforces Sol/high, one leaf, complete evidence, exact-head CI, controller-evidence review, protected integration and restart-safe issue reconciliation. Forty-two controller/fake-worker tests cover bootstrap/promotion routing and zero-account-review fixtures. The first real bounded worker run created PR #168; its protected integration remains pending. All 163 issue bodies, 115 parent links and 122 blocking relationships were synchronized against the initial contracts. The [setup report](../docs/implementation/setup-report.md) and [evidence record](../docs/implementation/setup-evidence.json) record the setup and activation state.
