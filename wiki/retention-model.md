...
Backups, point-in-time recovery logs, object versions, replicas, and other redundant recovery copies preserve availability but never extend an accepted retention period or artifact download window. When a record expires, automated deletion must remove it from every recoverable copy according to that deadline; a restored environment reapplies all elapsed deletions before it can serve data. Only an active, exact Legal Hold may suspend this process. See [[backend-architecture]].
...
School Data Export is not part of Release 1. The School export retention material below is preserved as later-release input governed by [ADR 0038](../docs/adr/0038-defer-school-data-export-to-a-later-release.md) and does not authorize implementation under the current Release 1 work graph. Release 1 still enforces the represented-School source-record retention, privacy, and access boundaries independently of any export product.

School Export History retains restricted versioned permission requests, decisions, ineligibility closures, waiting-period bypasses and metadata-only audit flags, grant offers, successor review and suspension, generation attempts and successful snapshot identity, category-level retention failures, completion, delivered multipart and checksum status, repeated-integrity failures, incomplete sessions, download-session outdated-warning acknowledgment, sharing, replacement linkage, priority and cancellation, failure, expiration, actor, selection, and date-range metadata for two years without exported payload. Superseded and schema-blocked School-wide preset versions and their repair provenance follow the same two-year period. The current School Manager sees all School activity and retained request history, a permissioned Coaching Staff member sees only their own activity, a former requester retains only their own explanation and final decision for two years, and governed privacy or audit personnel retain their separate projection. Public verification retains no submitted file, manifest, checksum, or identifying export metadata after the verification response. Non-secret verification keys, downloadable root-signed directory snapshots, rotation and root-transition history, revocation or retirement status, and effective dates remain indefinitely.
...
- Account Security History sign-ins, Clerk Reverification events, suspension-status events, and completed Account-export events are retained for two years and then deleted unless held. Longer Suspension Decision Record retention does not make that restricted record part of the Account-facing history.
...
- A Represented-School Data Export is an ephemeral authenticated set of checklist-generated CSV or PDF files, using ZIP only for multiple files or oversized output. Its seven-day deletion clock begins when the selected output is ready. Cancellation may delete pending or generated server-side files immediately, completed exports have no daily cap, and generation throttling, preflight warnings, cancellation, outdated status, and deletion do not change source-record retention. Payload-free School Export History follows its two-year period, non-secret signature verification keys remain indefinitely, downloaded copies cannot be recalled, and later public-key validation does not restore access to live School status.
...
Repeated-bypass audit classifications and compromise-notice delivery status remain in payload-free School Export History for two years. Invalidated signed-artifact history remains within that same two-year operational metadata period, while underlying source records keep their independently governed retention. Downloaded offline key-directory snapshots carry their own publication and last-check times; Docket retains no record of a user's local verification activity.

School Export History retains delivered-part status after revocation, integrity-review classification and overdue escalation, invalidation notices, bypass-warning acknowledgment, restricted investigation referrals, and verifier-release provenance for two years without payload. Shared public validation specifications, signed verifier releases, test vectors, verification keys, and trust-chain history remain available as long as needed to verify supported exports; local verification activity remains uncollected.

## Decision record

- **2026-09-04:** The project owner required database and object-storage recovery to preserve original retention and download-expiry deadlines rather than revive expired records.

- **2026-09-04:** The project owner retained permission-request, suspended-grant, sharing, and superseded School-preset metadata for two years, preserved signature keys indefinitely, and separated former-recipient cryptographic validation from live School status.

- **2026-09-04:** The project owner expanded payload-free School Export History to grant and delivery lifecycle metadata, retained it for two years, split Manager all-School from permissioned-staff own-activity views, and kept separate governed privacy and audit access.

- **2026-09-04:** The project owner defined retained School export artifacts as generated CSV or PDF with conditional ZIP, preserved indefinite non-secret signature verification, and kept the seven-day generated-file lifecycle independent of source records.

- **2026-09-04:** The project owner removed the daily completed School-export limit, retained abuse throttling for repetitive failure or cancellation, and confirmed that cancellation cannot recall already downloaded copies or alter source retention.
