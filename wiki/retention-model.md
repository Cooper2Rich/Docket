...
Infrastructure service telemetry is a separate, shorter-lived class: structured application logs and traces expire after 14 days, and aggregated service metrics expire after 90 days. They contain correlation identifiers and operational measurements but no credentials, Ballot contents, contact information, or sensitive request or projection payloads. Service telemetry cannot replace a domain audit, delivery record, or competitive provenance record whose longer policy applies.

Production logs, traces, metrics, and their backups remain in approved US locations throughout those retention periods. Public traffic is decrypted under the accepted Cloudflare US localization controls. Vendor control-plane metadata or support access is permitted only under reviewed contractual restrictions and does not authorize application payloads or telemetry to leave the accepted production-data boundary.
...
- Metadata-only Cloudflare break-glass approval and action provenance follows the seven-year privileged Platform Administrator period. It contains no token value and does not preserve external secrets after revocation.
...
- A confirmed-malware upload remains inaccessible and is deleted no later than 24 hours after detection unless an exact Legal Hold applies. A metadata-only security event may remain under the applicable routine operational or incident retention class, but it contains no malicious bytes, signed URL, or sensitive scanner detail exposed to the uploader.
