          ...
          ? {
              requestBody: {
                ...
                content: {
                  "application/json": {
                    schema: {
                      $ref: `#/components/schemas/${operation.inputSchema}`,
                    },
                  },
                },
              },
  ...
  const clients = sources
    .flatMap((source) =>
      source.operations.map((operation) => {
        ...
        const request =
          operation.inputLocation === "body"
            ? `{ method: ${JSON.stringify(operation.method.toUpperCase())}, path: ${JSON.stringify(operation.path)}, body: input }`
            : `{ method: ${JSON.stringify(operation.method.toUpperCase())}, path: ${JSON.stringify(operation.path)} + (query ? \`?\${query}\` : "") }`;
...
${query}    const response = await transport(${request});
