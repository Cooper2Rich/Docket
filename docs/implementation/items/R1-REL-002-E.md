# R1-REL-002-E: Rehearse disaster recovery and continuity objectives

Parent: R1-REL-002 · Order: 1130 · Owner: runtime · Stage: 08 Governance and release
...
- Completed and independently verified leaves: R1-REL-002-D.
- External/decision gates: none unique to this leaf; protected integration and review still apply.
- Read AGENTS.md, wiki/index.md, BUILD.md, CONTEXT.md and the applicable ADRs; then the scoped sources below. Future commands are created by the named foundation dependencies and are not currently implemented.

## Acceptance criteria

- **R1-REL-002-E/AC-01** — Demonstrate recovery point at most five minutes and recovery time at most 30 minutes through a real isolated restore/failover exercise with timed evidence and named operator.
- **R1-REL-002-E/AC-02** — Reconcile jobs, notifications, identity, artifacts, Legal Holds and elapsed deletions before enabling traffic; prove no duplicate results, resurrected expired records or stale authority.
- **R1-REL-002-E/AC-03** — Measure and report against 99.95% active-tournament and 99.9% other availability objectives with an explicit observation period; a rehearsal cannot claim an unobserved production SLO history.
- **R1-REL-002-E/AC-04** — For this slice, prove allowed and denied actors, stale version/authority, equivalent retry, conflicting concurrent action, transaction rollback and delivery failure at the authoritative boundary; persist only permitted data and enforce its existing retention/hold contract. For a user journey, exercise its real rendered route with loading, empty, error, denied, stale, mobile and keyboard states. Mark a category inapplicable only with a source-backed reason reviewed in the PR; an omitted case, fake UI, empty suite or unimplemented persistence is not a pass.
...
| --- | --- | --- | --- |
| item | `pnpm verify:item --id R1-REL-002-E --contract docs/implementation/queue-contract.json` | R1-FND-001-A | Exact acceptance-ID set; every required scenario has at least one executed assertion, zero failed/skipped/pending assertions, current code revision and contract digest, hashed evidence artifacts. |
...
Write machine evidence to `.ralph/evidence/R1-REL-002-E/acceptance.json` using [the verification protocol](../verification-protocol.md). The independent expected sets are stored in `docs/implementation/queue-contract.json`; the worker cannot reduce them.
