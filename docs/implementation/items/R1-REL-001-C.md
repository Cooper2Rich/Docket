# R1-REL-001-C: Verify supported browsers and accessible actor experiences

Parent: R1-REL-001 · Order: 1080 · Owner: workflows · Stage: 08 Governance and release
...
- Completed and independently verified leaves: R1-REL-001-B.
- External/decision gates: GATE-REVIEW.
- Read AGENTS.md, wiki/index.md, BUILD.md, CONTEXT.md and the applicable ADRs; then the scoped sources below. Future commands are created by the named foundation dependencies and are not currently implemented.

## Acceptance criteria

- **R1-REL-001-C/AC-01** — Run current and previous supported browser versions against the release actor journeys, including narrow-screen layout, keyboard-only navigation, focus, validation, denied/stale/loading/empty and unavailable states.
- **R1-REL-001-C/AC-02** — Automated accessibility checks and documented manual assistive-technology review meet the accepted WCAG 2.2 AA gate; real reviewer evidence is required where automation cannot establish conformance.
- **R1-REL-001-C/AC-03** — Record exact browser/build versions, cases and remaining findings. Unperformed manual review is pending, never a synthetic pass; any accepted exception needs the explicit release authority and recorded rationale.
- **R1-REL-001-C/AC-04** — For this slice, prove allowed and denied actors, stale version/authority, equivalent retry, conflicting concurrent action, transaction rollback and delivery failure at the authoritative boundary; persist only permitted data and enforce its existing retention/hold contract. For a user journey, exercise its real rendered route with loading, empty, error, denied, stale, mobile and keyboard states. Mark a category inapplicable only with a source-backed reason reviewed in the PR; an omitted case, fake UI, empty suite or unimplemented persistence is not a pass.
...
| --- | --- | --- | --- |
| item | `pnpm verify:item --id R1-REL-001-C --contract docs/implementation/queue-contract.json` | R1-FND-001-A | Exact acceptance-ID set; every required scenario has at least one executed assertion, zero failed/skipped/pending assertions, current code revision and contract digest, hashed evidence artifacts. |
...
Write machine evidence to `.ralph/evidence/R1-REL-001-C/acceptance.json` using [the verification protocol](../verification-protocol.md). The independent expected sets are stored in `docs/implementation/queue-contract.json`; the worker cannot reduce them.
