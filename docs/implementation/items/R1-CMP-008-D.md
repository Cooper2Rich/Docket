# R1-CMP-008-D: Sign off replacements and administrative No-Show outcomes

Parent: R1-CMP-008 · Order: 680 · Owner: competition · Stage: 05 Preliminary competition
...
- Completed and independently verified leaves: R1-CMP-008-C.
- External/decision gates: none unique to this leaf; protected integration and review still apply.
- Read AGENTS.md, wiki/index.md, BUILD.md, CONTEXT.md and the applicable ADRs; then the scoped sources below. Future commands are created by the named foundation dependencies and are not currently implemented.

## Acceptance criteria

- **R1-CMP-008-D/AC-01** — Rank and show all eligible same-School, same-event replacements under locked Ruleset timing and substitution rules; staff explicitly signs off after fresh validation and no automatic selection occurs.
- **R1-CMP-008-D/AC-02** — Before sign-off staff may restore the original arrival; afterward reversal requires Director correction, and a started round never restores the original assignee.
- **R1-CMP-008-D/AC-03** — If no eligible replacement exists, record the applicable reasoned bye or forfeit without fabricated opponent, Judge ballot or point value. Ruleset-authorized preliminary administrative points retain formula and input provenance; elimination outcomes reject all points.
- **R1-CMP-008-D/AC-04** — For this slice, prove allowed and denied actors, stale version/authority, equivalent retry, conflicting concurrent action, transaction rollback and delivery failure at the authoritative boundary; persist only permitted data and enforce its existing retention/hold contract. For a user journey, exercise its real rendered route with loading, empty, error, denied, stale, mobile and keyboard states. Mark a category inapplicable only with a source-backed reason reviewed in the PR; an omitted case, fake UI, empty suite or unimplemented persistence is not a pass.
...
| --- | --- | --- | --- |
| item | `pnpm verify:item --id R1-CMP-008-D --contract docs/implementation/queue-contract.json` | R1-FND-001-A | Exact acceptance-ID set; every required scenario has at least one executed assertion, zero failed/skipped/pending assertions, current code revision and contract digest, hashed evidence artifacts. |
...
Write machine evidence to `.ralph/evidence/R1-CMP-008-D/acceptance.json` using [the verification protocol](../verification-protocol.md). The independent expected sets are stored in `docs/implementation/queue-contract.json`; the worker cannot reduce them.
...
- [wiki/ballot-model.md](https://github.com/Cooper2Rich/Docket/blob/codex/release-1-issue-setup/wiki/ballot-model.md): Duplicate and conflicting submissions. Clauses: SRC-43f3ef1f-073, SRC-43f3ef1f-074, SRC-43f3ef1f-075, SRC-43f3ef1f-076, SRC-43f3ef1f-077.
...
- [wiki/tournament-directory-model.md](https://github.com/Cooper2Rich/Docket/blob/codex/release-1-issue-setup/wiki/tournament-directory-model.md): Purpose. Clauses: SRC-f290e4f2-028.
