# R1-CMP-013-C: Correct advancement before and after elimination starts

Parent: R1-CMP-013 · Order: 820 · Owner: competition · Stage: 06 Standings and eliminations
...
- Completed and independently verified leaves: R1-CMP-013-B.
- External/decision gates: none unique to this leaf; protected integration and review still apply.
- Read AGENTS.md, wiki/index.md, BUILD.md, CONTEXT.md and the applicable ADRs; then the scoped sources below. Future commands are created by the named foundation dependencies and are not currently implemented.

## Acceptance criteria

- **R1-CMP-013-C/AC-01** — Before any elimination starts, remove an ineligible qualifier, refill from the next eligible Entry under locked policy, recompute seeds/byes and withdraw/republish every affected unstarted artifact.
- **R1-CMP-013-C/AC-02** — After any elimination starts, deny refill and reseeding; preserve bracket lineage and record the applicable versioned bye, forfeit or disqualification with future effect.
- **R1-CMP-013-C/AC-03** — Emergency plan correction needs the accepted evidence, exact preview and Director approval; preserve old plans/fields, revalidate dependencies and publish minimized notices without tactical policy substitution.
- **R1-CMP-013-C/AC-04** — For this slice, prove allowed and denied actors, stale version/authority, equivalent retry, conflicting concurrent action, transaction rollback and delivery failure at the authoritative boundary; persist only permitted data and enforce its existing retention/hold contract. For a user journey, exercise its real rendered route with loading, empty, error, denied, stale, mobile and keyboard states. Mark a category inapplicable only with a source-backed reason reviewed in the PR; an omitted case, fake UI, empty suite or unimplemented persistence is not a pass.
...
| --- | --- | --- | --- |
| item | `pnpm verify:item --id R1-CMP-013-C --contract docs/implementation/queue-contract.json` | R1-FND-001-A | Exact acceptance-ID set; every required scenario has at least one executed assertion, zero failed/skipped/pending assertions, current code revision and contract digest, hashed evidence artifacts. |
...
Write machine evidence to `.ralph/evidence/R1-CMP-013-C/acceptance.json` using [the verification protocol](../verification-protocol.md). The independent expected sets are stored in `docs/implementation/queue-contract.json`; the worker cannot reduce them.
...
- [wiki/access-model.md](https://github.com/Cooper2Rich/Docket/blob/codex/release-1-issue-setup/wiki/access-model.md): Tournament operations delegation. Clauses: SRC-3aa2b898-185.
...
- [wiki/registration-model.md](https://github.com/Cooper2Rich/Docket/blob/codex/release-1-issue-setup/wiki/registration-model.md): Accommodation closure and retention. Clauses: SRC-751d83c0-096.
