...
- Completed and controller-verified leaves: R1-FND-005-A.
- External/decision gates: GATE-GITHUB.
- Read AGENTS.md, wiki/index.md, BUILD.md, CONTEXT.md and the applicable ADRs; then the scoped sources below. Future commands are created by the named foundation dependencies and are not currently implemented.

## Acceptance criteria

- **R1-FND-005-B/AC-01** — Read back main protection: all eleven current-head checks, required pull requests and resolved conversations, no force push/deletion, administrator enforcement, and no bypass actors are configured. GitHub approval count is zero under the accepted single-owner policy.
- **R1-FND-005-B/AC-02** — Prove a pull request lacking any required check cannot merge. Promote the exact completed bootstrap head once, verify its ancestry in main, and prove the bootstrap completion exception cannot authorize a later leaf.
- **R1-FND-005-B/AC-03** — Record public-repository and protection evidence without exposing secrets; an unavailable protection feature is an external blocker and never a reason to weaken required checks or use an administrator bypass.
